...
df_500 = graph.run(
  """MATCH (s:Station)-[i:IS_ON]->(:Line)
  WITH count(i) AS totalLines, s
  WHERE totalLines > 2
  MATCH (s)<-[:IS_WITHIN {distance : 500}]-(p)<-[t:SALE_IN]-(sl)
  WITH DISTINCT(sl) as sl,s,totalLines
  WITH  [x IN collect(sl) WHERE x.year = 1998 | x.price] as sales1998,[x IN collect(sl) WHERE x.year = 2008 | x.price] as sales2008,[x IN collect(sl) WHERE x.year = 2018 | x.price] as sales2018,s,totalLines
  RETURN s.name as stationName, totalLines,s.location.x as longitude, s.location.y as latitude, COALESCE(apoc.coll.avg(sales1998),0) as avg1998_500, COALESCE(apoc.coll.avg(sales2008),0) as avg2008_500,COALESCE(apoc.coll.avg(sales2018),0) as avg2018_500"""
).to_data_frame()
...
df_1000 = graph.run(
  ...
  WITH DISTINCT(sl) as sl,s,totalLines
  WITH  [x IN collect(sl) WHERE x.year = 1998 | x.price] as sales1998,[x IN collect(sl) WHERE x.year = 2008 | x.price] as sales2008,[x IN collect(sl) WHERE x.year = 2018 | x.price] as sales2018,s,totalLines
  RETURN s.name as stationName,totalLines, s.location.x as longitude, s.location.y as latitude, COALESCE(apoc.coll.avg(sales1998),0) as avg1998_1000, COALESCE(apoc.coll.avg(sales2008),0) as avg2008_1000,COALESCE(apoc.coll.avg(sales2018),0) as avg2018_1000"""
).to_data_frame()
...
df_combined = pd.merge(df_500, df_1000, on=['stationName'])
